...
export default function CustomerView() {
  ...
  const handleVote = (id, delta) => {
    setQueue((prev) =>
      prev
        .map((s) => (s.id === id ? { ...s, votes: s.votes + delta } : s))
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-[#F5F5F7] font-sans pb-44 relative overflow-hidden">
      ...
      >
        <div className="space-y-4 pb-24">
          ...
          {queue.length === 0 ? (
            <p className="text-center text-gray-500 text-sm italic mt-20">
              The queue is currently empty. Be the first to add a song!
            </p>
          ) : (
            queue.map((song, index) => (
              <div key={song.id} style={{ animationDelay: `${index * 50}ms` }}>
